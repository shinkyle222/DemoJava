...
public class PrioirtyQueueEx {
	...
	public static void main(String[] args) {
		...
		//객체가 우선순위를 가지게 하려면 comparable 메서드를
		//오버로딩해서 가지면 된다.
		...
		//홍길동이 사전적으로 뒤에 위치 => 양수
		System.out.println("홍길자".compareTo("홍길자"));
		//같을시에는 0이 나온다.
		
		Queue<User> queue = new PriorityQueue<>();
		
		//user의 객체 안에 들어있는 멤버변수에 순서를 보고 지정한다.
		...
		//주소값이 객체로 되어 정렬되어 나온다.
		System.out.println(queue.poll());
		System.out.println(queue.poll());
		System.out.println(queue.poll());
		System.out.println(queue.poll());
		
		//System.out.println("가장먼저나온객체의 이름:" + queue.poll().getName());
	}
}
